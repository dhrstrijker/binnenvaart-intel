...
export default function VesselPageContent({ vessel, similarVessels }: VesselPageContentProps) {
  ...
  const specPills: { label: string; value: string }[] = [];
  if (vessel.type) specPills.push({ label: "Type", value: vessel.type });
  if (vessel.length_m && vessel.width_m)
    specPills.push({ label: "Afmetingen", value: `${vessel.length_m} x ${vessel.width_m} m` });
  if (vessel.build_year) specPills.push({ label: "Bouwjaar", value: String(vessel.build_year) });
  if (vessel.tonnage) specPills.push({ label: "Tonnage", value: `${vessel.tonnage}t` });
    ...
    <article>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          ...
          {/* MarineTraffic Map */}
          <MarineTrafficMap className="h-[200px]" />
        </div>
        ...
        <div className="rounded-2xl bg-white p-6 shadow-lg ring-1 ring-slate-100">
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex shrink-0 gap-2">
              ...
              <button
                type="button"
                className="flex h-9 w-9 items-center justify-center rounded-lg ring-1 ring-slate-200 text-slate-400 transition-colors hover:text-red-500 hover:ring-red-200"
              ...
              <div className="relative">
                <button
                  type="button"
                  onClick={handleShare}
                  className="flex h-9 w-9 items-center justify-center rounded-lg ring-1 ring-slate-200 text-slate-400 transition-colors hover:text-cyan-600 hover:ring-cyan-200"
          ...
          {specPills.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-2">
              {specPills.map((pill) => (
                <span
                  key={pill.label}
                  className="rounded-full bg-slate-100 px-3.5 py-1.5 text-sm font-medium text-slate-700"
                >
                  {pill.value}
                </span>
              ))}
          ...
          {/* Broker comparison (multi-source) */}
          {multiSource && (
            <div className="mt-5">
              <p className="text-sm font-semibold text-slate-700">
                Bij {vessel.linked_sources!.length} makelaars
              </p>
              <div className="mt-2 space-y-2">
                {vessel.linked_sources!.map((ls) => (
                  <a
                    ...
                    className="flex items-center justify-between rounded-xl bg-slate-50 p-3 ring-1 ring-slate-100 transition-colors hover:ring-cyan-200"
                  >
                    <div className="flex items-center gap-2.5">
                      <span
                        className={`rounded-md px-2 py-0.5 text-xs font-semibold ${sourceColor(ls.source)}`}
                      >
          ...
          {!multiSource && (
            <div className="mt-5">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 text-sm font-semibold text-cyan-600 transition-colors hover:text-cyan-800"
              >
